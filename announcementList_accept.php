<?php
  ...
  require($serverdpphp);
    ...
    $sql_CountRecord = "SELECT COUNT(*) As totalRecords FROM announcement WHERE Status='Accepted'";
    ...
    //$sql_AAcceptedList = "SELECT * FROM announcement WHERE Status='Accepted' ";
    $sql_AAcceptedList = "SELECT * FROM announcement WHERE Status='Accepted' LIMIT $Offset, $recordsPerPage";
    $showAAcceptedList_query = mysqli_query($con, $sql_AAcceptedList);
    
    //search in Announcement
    if(isset($_POST['searchAnnouncement'])){
        $SearchInAnnouncement = $_POST['SearchInAnnouncement'];
        header("Location: ../ADMIN_2.0/announcementList_accept.php?PageNo=1&search={$SearchInAnnouncement}");  
    }
    if(isset($_GET['search'])){
        $bool = true;
        $SearchInAnnouncement = $_GET['search'];

        $sql_CountRecord = "SELECT COUNT(*) As totalRecords FROM announcement WHERE Status = 'Accepted' AND Subject LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Accepted' AND EventDetails LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Accepted' AND Author LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Accepted' AND DateStart LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Accepted' AND DatePosted LIKE '%".$SearchInAnnouncement."%' ";
        $CountRecord_query = mysqli_query($con, $sql_CountRecord);
        ...
        $sql_AAcceptedList = "SELECT * FROM announcement WHERE Status = 'Accepted' AND Subject LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Accepted' AND EventDetails LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Accepted' AND Author LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Accepted' AND DateStart LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Accepted' AND DatePosted LIKE '%".$SearchInAnnouncement."%' 
    ...
    if (isset($_GET['Remove_ID'])){
        ...
        $sql_RemoveAnnouncement = "UPDATE announcement SET Status = 'Removed' WHERE AnnouncementId = '".$Remove_ID."' ";
        $removeStatus_query = mysqli_query($con, $sql_RemoveAnnouncement);

        if($removeStatus_query){
            header("Location: announcementList_accept.php");
        }
    }
?>
...
<head>
    ...
    <title>Annoucement | Accepted</title>
                ...
                <div class="page" id="announcement-page">
                    ...
                    <div class="table-button">
                        <div class="drop-down">
                            <div class="select-dropdown">
                                <input type="text" placeholder="Accepted" class="text-box" readonly>
                                <div class="option">
                    ...
                    <div class="table-container" id="all-table">
                        <table>
                            ...
                            <?php
                                while($row = mysqli_fetch_assoc($showAAcceptedList_query)){ 
                            ?>
                            <tr>
                                ...
                                <td> <?php echo "<a style='background-color: rgb(218,24,14);' href='announcementList_accept.php?Remove_ID=".$row['AnnouncementId']."' class='btn' onclick='RemoveNotif()'> Remove </a> 
                        ...
                        <nav aria-label="Page navigation example">
                            ...
                            <li class="page-item" <?php if($PageNo <= 1){ echo "class='disabled'"; } ?> >
                                <a class="page-link" <?php if($PageNo > 1){
                                    if(isset($bool)){
                                        echo "href='announcementList_accept.php?PageNo={$PrevPage}&search={$SearchInAnnouncement}'";
                                    }
                                    else{
                                        echo "href='announcementList_accept.php?PageNo=$PrevPage'";
                                    }
                                } ?>>Previous</a>
                            </li>
                            ...
                            <li class="page-item" <?php if($PageNo >= $NoPages){
                                ...
                                <a class="page-link" <?php if($PageNo < $NoPages) {
                                    if(isset($bool)){
                                        echo "href='announcementList_accept.php?PageNo={$NextPage}&search={$SearchInAnnouncement}'";
                                    }
                                    else{
                                        echo "href='announcementList_accept.php?PageNo=$NextPage'";
                                    }
                                } ?>>Next</a>
                            </li>
                            ...
                            <?php if($PageNo > 1){
                                if(isset($bool) && $_GET['search'] != ''){
                                    echo "<li class='page-item'> <a class='page-link' href='announcementList_accept.php?PageNo=1&search={$SearchInAnnouncement}'>First Page</a> </li>";
                                    }
                                    else{
                                        echo "<li class='page-item'> <a class='page-link' href='announcementList_accept.php?PageNo=1'>First Page</a> </li>";
                                    }
                            } ?>
                            <?php if($PageNo < $NoPages){
                                if(isset($bool) && $_GET['search'] != ''){
                                    echo "<li class='page-item'> <a class='page-link' href='announcementList_accept.php?PageNo=$NoPages&search={$SearchInAnnouncement}'>Last Page</a> </li>";
                                    }
                                    else{
                                        echo "<li class='page-item'> <a class='page-link' href='announcementList_accept.php?PageNo=$NoPages'>Last Page</a> </li>";
                                    }
                            } ?>
                        </nav> 
                </div>
