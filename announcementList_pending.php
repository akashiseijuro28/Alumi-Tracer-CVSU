<?php
  ...
  require($serverdpphp);
    ...
    $sql_CountRecord = "SELECT COUNT(*) As totalRecords FROM announcement WHERE Status='Pending'";
    ...
    //$sql_APendingList = "SELECT * FROM announcement WHERE Status='Pending";
    $sql_APendingList = "SELECT * FROM announcement WHERE Status='Pending' LIMIT $Offset, $recordsPerPage";
    $showAPendingList_query = mysqli_query($con, $sql_APendingList); 

    //search in Announcement
    if(isset($_POST['searchAnnouncement'])){
        $SearchInAnnouncement = $_POST['SearchInAnnouncement'];
        header("Location: ../ADMIN_2.0/announcementList_pending.php?PageNo=1&search={$SearchInAnnouncement}");  
    }
    if(isset($_GET['search'])){
        $bool = true;
        $SearchInAnnouncement = $_GET['search'];

        $sql_CountRecord = "SELECT COUNT(*) As totalRecords FROM announcement WHERE Status = 'Pending' AND Subject LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Pending' AND EventDetails LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Pending' AND Author LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Pending' AND DateStart LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Pending' AND DatePosted LIKE '%".$SearchInAnnouncement."%' ";
        $CountRecord_query = mysqli_query($con, $sql_CountRecord);
        ...
        $sql_APendingList = "SELECT * FROM announcement WHERE Status = 'Pending' AND Subject LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Pending' AND EventDetails LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Pending' AND Author LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Pending' AND DateStart LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Pending' AND DatePosted LIKE '%".$SearchInAnnouncement."%' 
    ...
    if (isset($_GET['Accept_ID'])){
        ...
        $sql_AcceptAnnouncement = "UPDATE announcement SET Status = 'Accepted' WHERE AnnouncementId = '".$Accept_ID."' ";
        $acceptStatus_query = mysqli_query($con, $sql_AcceptAnnouncement);
        if($acceptStatus_query){
            header("Location: announcementList_pending.php");
        }
    }
    ...
    if (isset($_GET['Decline_ID'])){
        ...
        $sql_DeclineAnnouncement = "UPDATE announcement SET Status = 'Declined' WHERE AnnouncementId = '".$Decline_ID."' ";
        $declineStatus_query = mysqli_query($con, $sql_DeclineAnnouncement);

        if($declineStatus_query){
            header("Location: announcementList_pending.php");
        }
    }
...
<head>
    ...
    <title>Annoucement | Pending</title>
                ...
                <div class="page" id="announcement-page">
                    ...
                    <div class="table-button">
                        <div class="drop-down">
                            <div class="select-dropdown">
                                <input type="text" placeholder="Pending" class="text-box" readonly>
                                <div class="option">
                        ...
                        <div class="search-bar">
                            <form action="" method="POST">
                                <input name="SearchInAnnouncement" type="text" placeholder="Search ..."><button name="searchAnnouncement"><i class="fa fa-search"></i></button>
                            </form>
                    ...
                    <div class="table-container" id="all-table">
                        <table>
                            ...
                            <?php
                                while($row = mysqli_fetch_assoc($showAPendingList_query)){ 
                            ?>
                            <tr>
                                ...
                                <td> <?php echo "
                                <a style='background-color: rgb(181,230,29);' href='announcementList_pending.php?Accept_ID=".$row['AnnouncementId']."' class='btn' onclick='AcceptNotif()'> Accept </a> 
                                <a style='background-color: rgb(218,24,14);' href='announcementList_pending.php?Decline_ID=".$row['AnnouncementId']."' class='btn' onclick='DeclineNotif()'> Decline </a> 
                                <a href='announcement_view.php?View_ID=".$row['AnnouncementId']."' class='btn'> View </a>" ?>  </td>
                            </tr>
                            <?php
                                }
                            ?>                                                                                 
                        ...
                        <nav aria-label="Page navigation example">
                            ...
                            <li class="page-item" <?php if($PageNo <= 1){ echo "class='disabled'"; } ?> >
                                <a class="page-link" <?php if($PageNo > 1){
                                    if(isset($bool)){
                                        echo "href='announcementList_pending.php?PageNo={$PrevPage}&search={$SearchInAnnouncement}'";
                                    }
                                    else{
                                        echo "href='announcementList_pending.php?PageNo=$PrevPage'";
                                    }
                                } ?>>Previous</a>
                            </li>
                            ...
                            <li class="page-item" <?php if($PageNo >= $NoPages){
                                ...
                                <a class="page-link" <?php if($PageNo < $NoPages) {
                                    if(isset($bool)){
                                        echo "href='announcementList_pending.php?PageNo={$NextPage}&search={$SearchInAnnouncement}'";
                                    }
                                    else{
                                        echo "href='announcementList_pending.php?PageNo=$NextPage'";
                                    }
                                } ?>>Next</a>
                            </li>
                            ...
                            <?php if($PageNo > 1){
                                if(isset($bool) && $_GET['search'] != ''){
                                    echo "<li class='page-item'> <a class='page-link' href='announcementList_pending.php?PageNo=1&search={$SearchInAnnouncement}'>First Page</a> </li>";
                                    }
                                    else{
                                        echo "<li class='page-item'> <a class='page-link' href='announcementList_pending.php?PageNo=1'>First Page</a> </li>";
                                    }
                            } ?>
                            <?php if($PageNo < $NoPages){
                                if(isset($bool) && $_GET['search'] != ''){
                                    echo "<li class='page-item'> <a class='page-link' href='announcementList_pending.php?PageNo=$NoPages&search={$SearchInAnnouncement}'>Last Page</a> </li>";
                                    }
                                    else{
                                        echo "<li class='page-item'> <a class='page-link' href='announcementList_pending.php?PageNo=$NoPages'>Last Page</a> </li>";
                                    }
                            } ?>
                            </ul>
                        </nav> 
