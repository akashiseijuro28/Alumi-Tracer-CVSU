<?php 
  ...
  require($serverdpphp);
    ...
    $sql_Events = "SELECT a.*,b.* FROM announcementcreation a, announcement b WHERE a.category='Events' AND b.announcementId = a.Id AND b.Status='Accepted' order by Id DESC";
    $ShowEvents_Query = mysqli_query($con, $sql_Events);

    if(isset($_POST['sortEvents'])){
        $SubCategorySort = $_POST['SubCatSort'];
        if($SubCategorySort != "All"){
            $sql_Events = "SELECT a.*,b.* FROM announcementcreation a, announcement b WHERE a.category='Events' AND b.announcementId = a.Id AND b.Status='Accepted' AND a.subcat = '".$SubCategorySort."' order by Id DESC";
            $ShowEvents_Query = mysqli_query($con, $sql_Events);
        }
        else{
            $sql_Events = "SELECT a.*,b.* FROM announcementcreation a, announcement b WHERE a.category='Events' AND b.announcementId = a.Id AND b.Status='Accepted' order by Id DESC";
...
<head>
    ...
    <title>Annoucement | Events</title>
...
<body>
    <div class="container-2">
        ...
        <div class="content-box">
            <div class="content">
                <div class="head-page">
                    <h1>New and Events</h1>
                </div>
                <div class="wrapper">
                    <div class="row-1">
                        <div class="buttons">
                            <a href="announcement_news.php"><button style="background-color: #FC3134;">News</button></a>
                            <a href="announcement_events.php"><button style="background-color: #7BADEE;">Events</button></a> 
                        </div>
                        ...
                        <div class="cty">
                            <form action="" method="POST">
                                ...
                                <div class="button">
                                    <button name="sortEvents" class="btn btn-outline-success" type="submit">Sort</button>
                                </div>
                            </form>
                        </div>  
                    </div>
                    ...
                    <div class="row-2">
                        <?php
                        if ($ShowEvents_Query->num_rows > 0) {
                            while($row = mysqli_fetch_assoc($ShowEvents_Query)) {
