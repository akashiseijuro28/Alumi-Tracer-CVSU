...
FROM jobinfo WHERE year(DateHired)>0 group by  year(DateHired), Course, EmpStat order by year(DateHired)";
$sqltemp ="SELECT year(DateHired),
(SELECT COUNT(EmpStat)  WHERE   EmpStat = 'Employed') as TotalEmp
FROM jobinfo  WHERE  DateHired>0 and EmpStat = 'Employed' group by  year(DateHired) order by year(DateHired)";
