<?php
  ...
  require($serverdpphp);
    ...
    $sql_CountRecord = "SELECT COUNT(*) As totalRecords FROM announcement WHERE Status='Declined'";
    ...
    //$sql_ADeclinedList = "SELECT * FROM announcement WHERE Status='Declined'";
    $sql_ADeclinedList = "SELECT * FROM announcement WHERE Status='Declined' LIMIT $Offset, $recordsPerPage";
    $showADeclinedList_query = mysqli_query($con, $sql_ADeclinedList);

    //search in Announcement
    //search in Announcement
    if(isset($_POST['searchAnnouncement'])){
        $SearchInAnnouncement = $_POST['SearchInAnnouncement'];
        header("Location: ../ADMIN_2.0/announcementList_decline.php?PageNo=1&search={$SearchInAnnouncement}");  
    }
    if(isset($_GET['search'])){
        $bool = true;
        $SearchInAnnouncement = $_GET['search'];

        $sql_CountRecord = "SELECT COUNT(*) As totalRecords FROM announcement WHERE Status = 'Declined' AND Subject LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Declined' AND EventDetails LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Declined' AND Author LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Declined' AND DateStart LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Declined' AND DatePosted LIKE '%".$SearchInAnnouncement."%' ";
        $CountRecord_query = mysqli_query($con, $sql_CountRecord);
        ...
        $sql_ADeclinedList = "SELECT * FROM announcement WHERE Status = 'Declined' AND Subject LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Declined' AND EventDetails LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Declined' AND Author LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Declined' AND DateStart LIKE '%".$SearchInAnnouncement."%' 
            OR Status = 'Declined' AND DatePosted LIKE '%".$SearchInAnnouncement."%' 
...
<head>
    ...
    <title>Annoucement | Declined</title>
                ...
                <div class="page" id="announcement-page">
                    ...
                    <div class="table-button">
                        <div class="drop-down">
                            <div class="select-dropdown">
                                <input type="text" placeholder="Declined" class="text-box" readonly>
                                <div class="option">
                    ...
                    <div class="table-container" id="all-table">
                        <table>
                            ...
                            <?php
                                while($row = mysqli_fetch_assoc($showADeclinedList_query)){ 
                            ?>
                            <tr>
                        ...
                        <nav aria-label="Page navigation example">
                            ...
                            <li class="page-item" <?php if($PageNo <= 1){ echo "class='disabled'"; } ?> >
                                <a class="page-link" <?php if($PageNo > 1){
                                    if(isset($bool)){
                                        echo "href='announcementList_decline.php?PageNo={$PrevPage}&search={$SearchInAnnouncement}'";
                                    }
                                    else{
                                        echo "href='announcementList_decline.php?PageNo=$PrevPage'";
                                    }
                                } ?>>Previous</a>
                            </li>
                            ...
                            <li class="page-item" <?php if($PageNo >= $NoPages){
                                ...
                                <a class="page-link" <?php if($PageNo < $NoPages) {
                                    if(isset($bool)){
                                        echo "href='announcementList_decline.php?PageNo={$NextPage}&search={$SearchInAnnouncement}'";
                                    }
                                    else{
                                        echo "href='announcementList_decline.php?PageNo=$NextPage'";
                                    }
                                } ?>>Next</a>
                            </li>
                            ...
                            <?php if($PageNo > 1){
                                if(isset($bool) && $_GET['search'] != ''){
                                    echo "<li class='page-item'> <a class='page-link' href='announcementList_decline.php?PageNo=1&search={$SearchInAnnouncement}'>First Page</a> </li>";
                                    }
                                    else{
                                        echo "<li class='page-item'> <a class='page-link' href='announcementList_decline.php?PageNo=1'>First Page</a> </li>";
                                    }
                            } ?>
                            <?php if($PageNo < $NoPages){
                                if(isset($bool) && $_GET['search'] != ''){
                                    echo "<li class='page-item'> <a class='page-link' href='announcementList_decline.php?PageNo=$NoPages&search={$SearchInAnnouncement}'>Last Page</a> </li>";
                                    }
                                    else{
                                        echo "<li class='page-item'> <a class='page-link' href='announcementList_decline.php?PageNo=$NoPages'>Last Page</a> </li>";
                                    }
                            } ?>
                            </ul>
                        </nav> 
